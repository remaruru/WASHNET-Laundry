...
function AdminDashboard() {
  ...
  const fetchOrders = async (dateFilter = null) => {
    try {
      let url = apiConfig.endpoints.orders;
      if (dateFilter) {
        url += `?date=${dateFilter}`;
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get(apiConfig.endpoints.orderStatistics);
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const response = await axios.get(apiConfig.endpoints.employeeOverview);
  ...
  const handleDeleteOrder = async (orderId) => {
    if (window.confirm('Are you sure you want to delete this order?')) {
      try {
        await axios.delete(`${apiConfig.endpoints.orders}/${orderId}`);
        fetchOrders();
        fetchStatistics();
      } catch (error) {
  ...
  const handleUpdateOrder = async (updatedData) => {
    try {
      await axios.put(`${apiConfig.endpoints.orders}/${selectedOrder.id}`, updatedData);
...
function EditOrderModal({ order, onClose, onSave }) {
  ...
  const handleSubmit = (e) => {
    ...
    const cleanedData = {
      ...
      customer_email: formData.customer_email ? formData.customer_email.trim() : '',
