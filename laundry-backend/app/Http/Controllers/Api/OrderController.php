...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'customer_phone' => ['required', 'string', 'regex:/^[0-9]{10,11}$/'], // 10-11 digits only
            'customer_email' => 'nullable|email|max:255',
            'items' => 'required|array|min:1',
            'items.*.name' => 'required|string|max:255',
            'items.*.quantity' => 'required|integer|min:1|max:1000', // Reasonable max
            ...
            'pickup_date' => ['nullable', 'date', function ($attribute, $value, $fail) {
                if ($value && $value < date('Y-m-d')) {
                    $fail('The pickup date must be today or a future date.');
                }
            }],
            'delivery_date' => ['nullable', 'date', function ($attribute, $value, $fail) {
                if ($value && $value < date('Y-m-d')) {
                    $fail('The delivery date must be today or a future date.');
                }
            }],
            'delivery_method' => 'required|in:pickup,deliver',
        ]);
        
        // Custom validation for trimmed names
        $validator->after(function ($validator) use ($request) {
            $customerName = trim($request->customer_name ?? '');
            if (empty($customerName)) {
                $validator->errors()->add('customer_name', 'Customer name cannot be empty or only whitespace.');
            }
            ...
            if ($request->has('items')) {
                foreach ($request->items as $index => $item) {
                    ...
                    if (empty($itemName)) {
                        $validator->errors()->add("items.{$index}.name", 'Item name cannot be empty or only whitespace.');
                    }
                }
            }
        });
        ...
        $customerName = trim($request->customer_name);
        $customerPhone = preg_replace('/\D/', '', $request->customer_phone); // Numbers only
        $customerEmail = $request->customer_email ? trim($request->customer_email) : null;
        
        // Sanitize item names
        $items = $request->items;
        foreach ($items as &$item) {
            $item['name'] = trim($item['name']);
            $item['quantity'] = max(1, (int)$item['quantity']); // Ensure at least 1
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'customer_phone' => ['sometimes', 'required', 'string', 'regex:/^[0-9]{10,11}$/'], // 10-11 digits only
            'customer_email' => 'nullable|email|max:255',
            ...
            'delivery_date' => ['nullable', 'date', function ($attribute, $value, $fail) use ($request) {
                if ($value && $request->pickup_date && $value <= $request->pickup_date) {
                    $fail('The delivery date must be after the pickup date.');
                }
            }],
        ...
        // Sanitize customer name and phone if provided
        if ($request->has('customer_name')) {
            $updateData['customer_name'] = trim($request->customer_name);
        }
        if ($request->has('customer_phone')) {
            $updateData['customer_phone'] = preg_replace('/\D/', '', $request->customer_phone); // Numbers only
        }
        if ($request->has('customer_email')) {
            $updateData['customer_email'] = $request->customer_email ? trim($request->customer_email) : null;
        }
